...
if __name__ == '__main__':
    ...
    scale = torch.ones(num_dims)
    f = dynamics.GaussianDynamics1d(loc=loc, scale=scale)
    ...
    p0 = ds.MultivariateNormal(loc=torch.ones(num_dims), covariance_matrix=torch.diag(torch.ones(num_dims)))
    ...
    sign_p0 = ds.discretization_generator(dist=p0, nr_signature_points=10, compute_w2=True)
    ...
    eps = torch.norm(sign_p0.locs.unsqueeze(-2) - sign_p0.locs.unsqueeze(-3), dim=-1, p=torch.inf).fill_diagonal_(torch.inf).min(-1).values / 2
    regions = HyperRectangle.from_eps(sign_p0.locs, eps.unsqueeze(-1))
