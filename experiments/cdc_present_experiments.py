...
from wass_prop import multi_step, multi_step_empirical, SampledPath, AmbiguityBall, Path
...
paper_configs = dict(
    ...
    double_spiral_no_init_amb = ExpConfig(
        dynamics_type='DoubleSpiral2dDynamics',
        dynamics_setting=0,
        num_locs=10,
        num_samples=100,
        num_time_steps=10,
        radii=RadiiConfig(initial=0., noise=0.01),
        dynamics_plot=PlotConfig(xlim=[-2., 2.], ylim=[-1., 1.], figsize=(13, 8)),
        path_plot=PlotConfig(xlim=[-1., 1.], ylim=[-1., 1.], figsize=(9, 8)),
    ),
...
def illustrate(name: str):
    ...
    plot.save(save_by=f"{save_by}_samples_true", save=args.save)
...
if __name__ == '__main__':
    ...
    illustrate("double_spiral_no_init_amb")
